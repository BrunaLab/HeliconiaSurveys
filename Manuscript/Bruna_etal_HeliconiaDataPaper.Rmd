...
heliconia_data <- read_csv("./data_clean/HA_survey_with_Zombies.csv", col_types = cols())

# Number of rows in the survey file
file_2_rows <- nrow(heliconia_data)+1 
file_2_size <- (file.info("./data_clean/HA_survey_with_Zombies.csv")$size)/1000000
# heliconia_data$code.notes

# No. of plants per plot
plants_per_plot <- heliconia_data %>% 
  select(HA_ID_Number, plot, plotID, habitat, bdffp_reserve_no, code.notes) %>% 
  group_by(plotID) %>% 
  drop_na("plotID") %>% 
...
uly_per_plot <- heliconia_data %>% 
  select(HA_ID_Number, plot, plotID, habitat, bdffp_reserve_no, code.notes) %>% 
  filter(code.notes=="ULY (3)") %>% 
  group_by(plotID) %>% 
  summarise(count_uly=sum(n_distinct(HA_ID_Number)))

uly_per_yr_plot <- heliconia_data %>% 
  select(HA_ID_Number, year, plot, plotID, habitat, bdffp_reserve_no, code.notes) %>% 
  # filter(year==1999) %>% 
  filter(code.notes=="ULY (3)") %>% 
...
missing_per_plot <- heliconia_data %>% 
  select(HA_ID_Number, plot, plotID, habitat, bdffp_reserve_no, code.notes) %>% 
  filter(code.notes=="plant missing (60)") %>% 
  group_by(plotID) %>% 
...
a.  **Technicians:** Paulo Rubim (2007-2012),  
    Maria Beatriz Nogueira (2002), Maria Rosa Darrigo
    (2002-2003), Cris Follman Jurinitz (2003), Simone Benedet (2004).

b.  **BDFFP Staff:** Osmaildo Ferreira da Silva, Francsico
