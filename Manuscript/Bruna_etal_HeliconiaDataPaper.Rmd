...
2. **Abstract**: The data set covers 12 years (1997-2009) of annual
censuses of 13 populations of the Amazonian understory herb *Heliconia
...
\noindent  
D. **Key words:** Amazon, Brazil, deforestation, demography,
...
> *Heliconia acuminata* leaves are sometimes damaged by the hispine
> beetle *Cephaloleia nigriceps* Baly [@staines2014], but the amount of
> leaf tissue removed is minimal [@bruna2002a; @bruna2005]. However, the
...
> predation is very low and that *H. acuminata* has no seed bank
...
a.  **Technicians:** Paulo Rubim (2007-2012), Maria Rosa Darrigo
    (2002-2003), Simone Benedet (2004), Cris Follman Jurinitz (2003),
    Maria Beatriz Nogueira (2002), Sylvia Heredia (2001-2002).

b.  **BDFFP Staff ("mateiros"):** Osmaildo Ferreira da Silva, Francsico
    Marques, Alaercio Marajó dos Reis, João De Deus Fragata, Romeu
    Cardoso.

c.  **Student Field Assistants:** Wesley Dáttilo da Cruz (2007),
...
5.  **Proprietary restrictions:** None. However, we request that authors
...
3.  Bruna, E. M. 2002. Effects of forest fragmentation on *Heliconia
...
Table1 <- read_csv("./Manuscript/table_2.csv", col_names = TRUE) %>% 
  replace_na(list("Variable" = "",	"Definition" = "",	"Storage type" = "",	"List and definition of variable codes" = "",	"Units, range, and precision"=""))
Table1<-as.data.frame(Table1)
# Table1_names <- c("Variable",	"Definition",	"Storage type",	"List & definition of variable codes",	"Units, range, & precision") 
...
```{r Fig1, fig.align='center',fig.cap="Satellite image of the BDFFP landscape showing the location of the *Heliconia* Dempographic Plots.",fig.height = 3,fig.width=3}
plot(pressure)
```

```{r Fig2, fig.align='center',fig.cap="Population Structure.",fig.height = 3,fig.width=3}
plot(pressure)
```
