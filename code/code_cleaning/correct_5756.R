...
# 1684 / 1864 -------------------------------------------------------------
  ...
  ha_data<-ha_data %>%
    mutate(shts=if_else((plot == 5756 & 
                           tag_number == 1684 & 
                           year==2007),1,shts)) %>% 
    mutate(ht=if_else((plot == 5756 & 
                         tag_number == 1684 & 
                         year == 2007),14,ht)) %>% 
    mutate(code=if_else((plot == 5756 & 
                           tag_number == 1684 & 
                           year == 2007),"sdlg",code))
...
# 332 / 933 ---------------------------------------------------------------
  ...
  # Subsequent years have 933 (which should be dead) with size measurements
  # that are clearly 332. I think someone changed the tag without recording
  # that they did so, otherwise the subsequent years would have discovered the 
...
# 1789 --------------------------------------------------------------------
  # 1789 in C2: appears the number was recorded incorrectly. Delete it.
  delete1789 <- ha_data %>%
    filter(plot == 5756 &
             tag_number == 1789 &
             row == "C" &
...
# 1290 / 1686 -------------------------------------------------------------
  # 1290 was missing in 07, appears top have been re-tagged as 1686
  # add the 2007 data incorrectly recorded as 1864
  #  also fixed missing shts in 2008
  ...
  mutate(shts=if_else((plot == 5756 &
    ...
    mutate(code=if_else((plot == 5756 & 
                           tag_number == 1686 & 
                           year == 2004),"ULY",code)) %>% 
        mutate(code=replace(code,(plot == 5756 & 
                                    tag_number == 1686 & 
                                    year == 2007),NA))
...
# 976 ---------------------------------------------------------------------
  # 976 in on B7/C7 border, so recorded in C7 in 2 yrs. 
  # add 06 and 07 measurements to B7 and delete C7
  ha_data$ht[ha_data$plot == 5756 &
               ha_data$row == "B" &
               ha_data$column == "7" &
...
# 439 / 842 / 1231 / 1285 / 1608
  ...
  ha_data<-ha_data %>%
    ...
    mutate(code=replace(code, plot==5756 & 
                          year==2004 & 
                          tag_number==1285,"ULY")) %>% 
    mutate(shts=replace(shts, plot==5756 & 
                          year==2004 & 
                          tag_number==1285,3)) %>% 
    mutate(ht=replace(ht, plot==5756 & 
                        year==2004 & 
                        tag_number==1285,68)) %>% 
    mutate(code=replace(code, plot==5756 & 
                          year==2005 & 
...
# 424 --------------------------------------------------------------------  
  ...
  # 1726 --------------------------------------------------------------------
    # 1726 was a seedling in 2000, ht correction in 2006
    ha_data<-ha_data %>%
      mutate(code=replace(code, plot==5756 & 
                            year==2007 & 
                            tag_number==1726,"sdlg")) %>% 
      mutate(code=replace(code, plot==5756 & 
                            year==2008 & 
                            tag_number==1726,"dead")) 
