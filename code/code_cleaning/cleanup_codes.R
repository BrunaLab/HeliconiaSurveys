cleanup_codes <- function(ha_data) {
  ...
  ha_data <- ha_data %>%
    mutate(notes = if_else(code == "not on list. must be313",
      "is it 133?", notes
    )) %>%
    mutate(notes = if_else(code == "1 new infl + 1 old infl",
      "1 new infl + 1 old infl", notes
    )) %>%
    mutate(notes = if_else(code == "not on list. must be96",
      "is it 96?", notes
    )) %>%
    ...
    mutate(notes = if_else(code == "this number belongs to a pvc in a6",
      "this no belongs to pvc in a6", notes
    )) %>%
    mutate(notes = if_else(code == "pvc stake not on list",
      "pvc stake not on list", notes
    )) %>%
    ...
    mutate(notes = if_else(code == "horrible treefall in plot",
      "horrible treefall in plot", notes
    )) %>%
    mutate(notes = if_else(code == "treefall in plot",
      "treefall in plot", notes
    )) %>%
    mutate(notes = if_else(code == "horrible treefall in plot, can barely make it in",
      "horrible treefall in plot, can barely make it in",
      notes
    )) %>%
    mutate(notes = if_else(code == "lots of small branchfalls in plot",
      "lots of small branchfalls in plot", notes
    )) %>%
    mutate(notes = if_else(code == "plot is 50% treefall",
      "plot is 50% treefall", notes
    )) %>%
    ...
    mutate(code = if_else(code == "under fallen trunk",
      "under treefall", code
    )) %>%
    mutate(code = na_if(code, "under trunk")) %>%
    mutate(code = na_if(code, "under treefall")) %>%
    mutate(code = na_if(code, "under fallen trunk")) %>%
    mutate(code = if_else(code == "under branch" |
      code == "under branch), must have lost tag",
    ...
    mutate(code = if_else(code == "not on list. must be313",
      "not on list", code
    )) %>%
    mutate(code = if_else(code == "not on list. must be96",
      "not on list", code
    )) %>%
    ...
    mutate(code = if_else(code == "Dead, Not on List",
      "dead and not on list", code
    )) %>%
    mutate(code = if_else(code == "dead, not on list",
      "dead and not on list", code
    )) %>%
    ...
    mutate(code = if_else(code == "pvc stake not on list",
      "not on list", code
    )) %>%
