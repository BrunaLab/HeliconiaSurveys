...
# 2003 data entry: spreadsheet offset -------------------------------------
  ...
  # can't just assign to the shifted number - a few of them need to be corrected
  # manually after review. The first two rows add the value to now correct 
  ...
  ha_data <- ha_data %>% 
  mutate(code=replace(code, plot==5753 & 
                        tag_number==229 & 
                        year>2003, NA)) %>% 
    mutate(code=replace(code, plot==5753 & 
                          tag_number==248 &
...
# 319 ---------------------------------------------------------------------
  ...
  ha_data <- ha_data %>% 
    mutate(shts=replace(shts, plot==5753 & 
                          tag_number==319 & 
                          year==1999, NA)) %>% 
    mutate(ht=replace(ht, plot==5753 & 
                        tag_number==319 & 
                        year==1999, NA)) %>% 
    mutate(shts=replace(shts, plot==5753 & 
                          tag_number==319 & 
    ...
    mutate(shts=replace(shts, plot==5753 & 
                          tag_number==319 & 
                          year==2001, 1)) %>% 
    mutate(ht=replace(ht, plot==5753 & tag_number==319 & 
                        year==2001, 14)) %>% 
    mutate(shts=replace(shts, plot==5753 & 
                          tag_number==319 &
                          year==2002, 2)) %>% 
  ...
  # 293 in wrong location in database; correcting both 2003 & other years here
  fixes_2003<-fixes_2003 %>%
    mutate(new_row=replace(new_row, new_tag==293, "D")) 
  
  ha_data <- ha_data %>% 
    mutate(row=replace(row, plot==5753 & 
                         tag_number==293, "D")) %>% 
    mutate(y_09=replace(y_09, plot==5753 & 
                          tag_number==293, NA)) %>% 
    mutate(x_09=replace(x_09, plot==5753 & 
                          tag_number==293, NA)) %>% 
    mutate(shts=replace(shts, plot==5753 & 
                          tag_number==293 &
                          year==1999, 2)) %>% 
...
# ADD THE CORRECTED 2003 DATA BACK IN -------------------------------------
...
# 269 / 311 ---------------------------------------------------------------
  ...
  ha_data<-ha_data %>%
    ...
    mutate(code=replace(code, plot==5753 &
                          year==2003 &
                          tag_number==311,NA)) %>% 
    mutate(infl=replace(infl, plot==5753 & 
                          year==2003 & 
                          tag_number==311,NA)) %>% 
    mutate(shts=replace(shts, plot==5753 & 
                          year==2000 & 
...
# 108 ---------------------------------------------------------------------
  # Tag no. 108
  ha_data$code[ha_data$plot == 5753 &
                 ha_data$tag_number == 108 & 
                 ha_data$year == 2005] <- NA
# 841 ---------------------------------------------------------------------
  # tag_no 841
  ha_data <- ha_data[!(ha_data$plot == 5753 & 
                         ha_data$tag_number == 319 & 
                         ha_data$row == "D"), ]
# 412 ---------------------------------------------------------------------
...
# 325 ---------------------------------------------------------------------
  ha_data<-ha_data %>%
    mutate(code=replace(code, plot==5753 & 
                          year==2008 & 
                          tag_number==325, NA))
# 28 / 32 / 39 / 76 / 97 / 194 / 222 / 227 / 245 / 260 / 267 / 325 --------
  ha_data<-ha_data %>%
    mutate(code=replace(code, plot==5753 & 
                          year==2008 & 
                          tag_number==28, NA)) %>% 
