find_dupe_tags <- function(ha_data) {
  ...
  if (nrow(dupe_simplified) == 0) {
    ...
    these plants has been saved here: `data_review/dupe_heliconia_tags.csv`.
  ...
  write.csv(dupe_simplified, "./data/data_review/dupe_heliconia_tags.csv", row.names = FALSE)
