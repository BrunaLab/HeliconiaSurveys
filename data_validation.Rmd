...
ha <- read_csv(here("data_clean", "Ha_survey_with_Zombies.csv"))
```
...
ha %>%
  ...
  col_vals_lte(vars(n), 1,
              preconditions = ~ . %>%
                group_by(HA_ID_Number, year) %>%
                count(),
              label = "duplicated ID numbers") %>% 
  ...
  col_vals_lt(vars(shts), 3, na_pass = TRUE,
              preconditions = ~. %>% filter(code.notes == "sdlg (1)"),
              label = "seedlings < 3 shoots") %>% 
  col_vals_lt(vars(ht), 20, na_pass = TRUE,
              preconditions = ~. %>% filter(code.notes == "sdlg (1)"),
              label = "seedlings < 20cm") %>% 
