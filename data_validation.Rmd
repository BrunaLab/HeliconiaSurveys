...
```{r load-data}
ha <- read_csv(
  here("data/data_clean", "HDP_1997_2009.csv")
)
ha_plots <- read_csv(
  here("data/data_clean", "HDP_plots.csv")
)
```
...
ha %>% 
  ...
  labs(y = "Proportion missing values for height")
...
ha_change <- 
  ...
  select(plot, subplot, plant_id, year, shts_prev, shts, shts_diff, shts_pc, ht_prev, ht, ht_diff, ht_pc, everything()) %>% 
  ungroup()
```
...
ha %>%
  ...
  col_vals_in_set(vars(subplot), unlist(lapply(LETTERS[1:10], paste0, 1:10)),
                  actions = al_strict) %>%
  col_vals_in_set(vars(plot), ha_plots$plot,
                  actions = al_strict) %>%
