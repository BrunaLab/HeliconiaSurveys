...
We use the R package [`pointblank`](https://rich-iannone.github.io/pointblank/) to review and validate the plot-level descriptors (`HDP_plots.csv`) and clean demographic data set (`heliconia_data_clean.csv`) in preparation for archiving in Dryad and publication in Bruna et al. (2023). The report below includes:
...
ha <- read_csv(
  ...
  here("data", "data_clean", "heliconia_data_clean.csv")
