...
(function (factory) {
  ...
  var saved_offsetX = 0;
  var saved_offsetY = 0;
  // Value of current zoom ratio
  var current_zoom = 0;
  // Use current ratio (or use user zoom input)
  var useExistingRatio = false;
  ...
  Viewer.prototype = {
    ...
    build: function () {
      ...
      // Retrieve sticky ratio from stored value ( do not round the decimal )
      this.$stickyratio = $viewer.find('.stickyratio');
      current_zoom = this.$stickyratio.text();
      ...
      if (!options.playable) {
          $toolbar.find('li[class*=viewer-play]').addClass(CLASS_INVISIBLE).appendTo($toolbar);
      }
      ...
      if (!options.downloadable) {
          $toolbar.find('li[class*=viewer-download]').addClass(CLASS_INVISIBLE).appendTo($toolbar);
      }
    ...
    initList: function () {
      ...
      this.$images.each(function (i) {
        ...
        // If FT image is lazy loaded, it may have a loading spinner as src. So, retrieve the actual data
        var dataSrcFromFTLazyLoading = this.getAttribute('data-src');
        if(dataSrcFromFTLazyLoading) {
        	src = dataSrcFromFTLazyLoading;
        }
    ...
    renderList: function (index) {
      ...
      var outerWidth = width + 11; // 11 pixels adjustment for `margin-left` width and border
    ...
    initImage: function (callback) {
      ...
      getImageSize($image[0], $.proxy(function (naturalWidth, naturalHeight) {
        ...
        var topMargin;
        if (useExistingRatio) {
			topMargin = 0; // margin calculated later using stored offsets
        } else {
	        if(options.fittowidth) {
				if(viewerWidth - width > 300) {
					topMargin = 65;
				} else {
					topMargin = (viewerWidth - width) / 2;
				}
			} else {
				topMargin = (viewerHeight - height) / 2;
			};
	  	};
    ...
    keydown: function (e) {
      ...
      switch (which) {
        ...
        // View previous (Key: ←)
        case 37:
          pqga.pushEventToGA("imageViewerLeftOrRightKeyDown");
          this.prev();
          break;
    ...
    hide: function () {
      ...
      if (this.isViewed && options.transition) {
        ...
        // Viewer being closed - store current ratio in variable as it gets reset in zoom()
        var temp_current_zoom = current_zoom; 
        this.zoomTo(0, false, false, true);
        this.$stickyratio.html(temp_current_zoom);
      } else {
    ...
    view: function (index) {
      ...
      var link = $('#imageDownloadLink');
      link.attr("href", this.$image[0].src);
      ...
      this.$element.one(EVENT_VIEWED, $.proxy(function () {
        ...
        $title.html(alt);
        // Load ratio from hidden variable and force use 
        useExistingRatio = true;
        if (current_zoom > 0) {
			this.move(saved_offsetX,saved_offsetY);
        }
        current_zoom = this.$stickyratio.text();
        this.zoom(current_zoom, true);
      }, this));
    ...
    zoomTo: function (ratio, hasTooltip, _event, _zoomable) {
      ...
      if (isNumber(ratio) && this.isViewed && !this.isPlayed && (_zoomable || options.zoomable)) {
        ...
        if (ratio > 0.95 && ratio < 1.05) {
         ...
         if(useExistingRatio == true && current_zoom != 0) {
        	ratio = current_zoom;
        	useExistingRatio = false;
        } else {
        	// When user is hitting on + ,- symbols use zoom ratio as -1,+1
        	// current zoom = 0 on first page load - use computed ratio & not the stored one
        	current_zoom = ratio;
        }
        this.$stickyratio.html(current_zoom);
    ...
    toggleNavbar: function () {
		$('#toggleNavbarIcon').toggleClass('uxf-down-open-large uxf-up-open-large');					
		this.$navbar.slideToggle();
		if($('#toggleNavbarIcon').hasClass('uxf-down-open-large')){
			this.renderList();
		}
    },
  ...
  Viewer.DEFAULTS = {
    ...
    // Define the ratio when zoom the image
    zoomRatio: 0.1,
    ...
    '<div class="viewer-container">' +     
      ...
      '<div class="viewer-footer">' +        
        '<ul class="viewer-toolbar">' +
          '<li class="viewer-prev" data-action="prev" title="Previous image"></li>' +  
          '<li class="viewer-zoom-in hidden-xs" data-action="zoom-in" title="Zoom in"></li>' +
          '<li class="viewer-zoom-out hidden-xs" data-action="zoom-out" title="Zoom out"></li>' +
          '<li class="viewer-rotate-left hidden-xs" data-action="rotate-left" title="Rotate left"></li>' +
          '<li class="viewer-rotate-right hidden-xs" data-action="rotate-right" title="Rotate right"></li>' +
          '<li class="viewer-one-to-one" data-action="one-to-one" title="Scale one to one"></li>' +
          '<li class="viewer-reset" data-action="reset" title="Reset"></li>' +          
          '<li class="viewer-play" data-action="play" title="Play slideshow"></li>' +
          '<li class="viewer-flip-horizontal" data-action="flip-horizontal" title="Flip horizontal"></li>' +
          '<li class="viewer-flip-vertical" data-action="flip-vertical" title="Flip vertical"></li>' +
          '<li class="viewer-download" title="Download image"><a id="imageDownloadLink" href="#" target="_blank" download><span class="uxf-icon uxf-download"></span></a></li>' +
          '<li class="viewer-next" data-action="next" title="Next image"></li>' +
        '</ul>' +
        '<div class="toggle-thumbs hidden-xs"><span id="toggleNavbarIcon" class="uxf-icon uxf-down-open-large" data-action="toggleNavbar"></span></div>' +
