correct_2206 <- function(ha_data) {
 ...
 # 70 was mismeasured in missing in 2003 and missing in 2004. I think the tag 
  ...
  # originally 70. 
  # Easiest way to fix is to add the sdlg year data to 45, then delete 70
  ha_data<-ha_data %>%
    mutate(shts=replace(shts, plot==2206 & year==2003 & tag_number==45,1)) %>% 
    mutate(ht=replace(ht, plot==2206 & year==2003 & tag_number==45,14)) %>% 
    mutate(code=replace(code, plot==2206 & year==2003 & tag_number==70,"sdlg")) 
